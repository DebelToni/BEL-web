  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .zoom-controls {{
      ...
      padding: 8px;
      border-radius: 12px;
      border: 1px solid rgba(63,53,46,0.16);
      background: rgba(255,255,255,0.92);
      box-shadow: 0 10px 24px rgba(33, 27, 23, 0.16);
    }}
    .zoom-btn {{
      min-width: 36px;
      height: 36px;
      border: 1px solid rgba(63,53,46,0.2);
      border-radius: 8px;
      background: #ffffff;
      color: var(--ink);
      font: inherit;
    ...
    @media (max-width: 980px) {{
      .shell {{
        ...
        grid-template-rows: minmax(280px, 52dvh) minmax(0, 1fr) minmax(0, 1fr);
        gap: 10px;
        padding: 10px;
      }}
      .left, .right {{ padding: 12px; }}
      .left h1 {{ font-size: 22px; }}
      .left h2, .right h2 {{ font-size: 16px; }}
      .right h3 {{ font-size: 15px; margin-top: 12px !important; }}
      .left p, .right p,
      .left li, .right li,
      .left label, .right label,
      .left input, .right input,
      .left select, .right select,
      .left button, .right button {{ font-size: 13px; line-height: 1.35; }}
      .left input[type="search"], .left select {{ min-height: 34px; padding: 8px 10px; }}
      .left .controls {{ gap: 10px; margin: 12px 0 14px; }}
      .left .control {{ gap: 6px; }}
      .left .legend {{ gap: 6px; }}
      .left .legend-item {{ gap: 8px; }}
      .left .legend-item strong {{ font-size: 12px; }}
      .left .legend-item span {{ font-size: 11px; }}
      .right .chips {{ gap: 6px; margin: 8px 0; }}
      .right .chip {{ padding: 4px 8px; font-size: 11px; }}
      .right .connections {{ gap: 6px; margin-top: 10px; }}
      .right .connection {{ padding: 8px 10px; }}
      .right .connection strong {{ font-size: 12px; }}
      .right .connection span {{ font-size: 11px; }}
      .stat {{ font-size: 12px; }}
      .zoom-controls {{
        top: auto;
        right: 12px;
      ...
      .zoom-level {{ min-width: 48px; font-size: 11px; }}
      .canvas-wrap {{ height: 100%; }}
    }}
    @media (max-width: 640px) {{
      .shell {{
        grid-template-rows: minmax(240px, 46dvh) minmax(180px, 1fr) minmax(180px, 1fr);
      }}
      .left, .right {{ padding: 10px; }}
      .left h1 {{ font-size: 20px; }}
      .left h2, .right h2 {{ font-size: 15px; }}
      ...
      .left input, .right input,
      .left select, .right select,
      .left button, .right button {{ font-size: 12px; line-height: 1.3; }}
      .left input[type="search"], .left select {{ min-height: 32px; padding: 7px 9px; }}
      .left .controls {{ gap: 8px; margin: 10px 0 12px; }}
      .left .legend-item span {{ font-size: 10px; }}
      .right .chip {{ font-size: 10px; padding: 4px 7px; }}
      .right .connection strong {{ font-size: 11px; }}
      .right .connection span {{ font-size: 10px; }}
...
<body>
  <div class="shell">
    ...
    <main class="panel center">
      <div class="zoom-controls" aria-label="Graph zoom controls">
        ...
        <button id="zoom-reset" class="zoom-btn" type="button" aria-label="Reset zoom">100%</button>
    ...
    <aside class="panel right">
      ...
      <p id="detail-meta" class="muted">Щракни върху възел, за да видиш описание, мотивен профил и всички връзки.</p>
      <div id="detail-description"></div>
      <div id="detail-chips" class="chips"></div>
      <div id="detail-analysis" class="muted"></div>
      <h3 style="margin-top: 18px;">Всички връзки (по сила)</h3>
  ...
  <script>
    ...
    const connectionStrengthByWork = new Map();
    const selectedState = {{ workId: null }};
    const BASE_GRAPH_WIDTH = 1240;
    const BASE_GRAPH_HEIGHT = 1000;
    ...
    edges.forEach(edge => {{
      if (!connectionStrengthByWork.has(edge.source)) connectionStrengthByWork.set(edge.source, new Map());
      if (!connectionStrengthByWork.has(edge.target)) connectionStrengthByWork.set(edge.target, new Map());
      connectionStrengthByWork.get(edge.source).set(edge.target, edge.weight);
      connectionStrengthByWork.get(edge.target).set(edge.source, edge.weight);
    }});

    function centerGraph() {{
      canvasWrap.scrollLeft = Math.max(0, (graph.clientWidth - canvasWrap.clientWidth) / 2);
      canvasWrap.scrollTop = Math.max(0, (graph.clientHeight - canvasWrap.clientHeight) / 2);
    }}

    function applyZoom(nextZoom, preserveCenter = true) {{
      const clampedZoom = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, nextZoom));
      const previousZoom = zoomState.level;
      const previousWidth = BASE_GRAPH_WIDTH * previousZoom;
      const previousHeight = BASE_GRAPH_HEIGHT * previousZoom;

      const focusX = preserveCenter
        ? (canvasWrap.scrollLeft + canvasWrap.clientWidth / 2) / Math.max(previousWidth, 1)
        : 0.5;
      const focusY = preserveCenter
        ? (canvasWrap.scrollTop + canvasWrap.clientHeight / 2) / Math.max(previousHeight, 1)
        : 0.5;

      zoomState.level = clampedZoom;
      const width = Math.round(BASE_GRAPH_WIDTH * clampedZoom);
      const height = Math.round(BASE_GRAPH_HEIGHT * clampedZoom);
      graph.style.width = `${{width}}px`;
      graph.style.height = `${{height}}px`;

      canvasWrap.scrollLeft = Math.max(0, width * focusX - canvasWrap.clientWidth / 2);
      canvasWrap.scrollTop = Math.max(0, height * focusY - canvasWrap.clientHeight / 2);

      const zoomPercent = `${{Math.round(clampedZoom * 100)}}%`;
      zoomLevel.textContent = zoomPercent;
      zoomResetBtn.textContent = zoomPercent;
      zoomOutBtn.disabled = clampedZoom <= ZOOM_MIN + 0.0001;
      zoomInBtn.disabled = clampedZoom >= ZOOM_MAX - 0.0001;
    }}
    ...
    function selectWork(workId) {{
      ...
      // Keep visual emphasis synced with the current selection.
      updateGraph();
    }}
    ...
    function updateGraph() {{
      ...
      const selectedId = selectedState.workId;
      const selectedConnections = selectedId ? (connectionStrengthByWork.get(selectedId) || new Map()) : null;
      ...
      edgeElements.forEach(({{ edge, element }}) => {{
        ...
        if (connectedToSelection) {{
          element.classList.remove('dimmed');
          element.style.opacity = (0.22 + edge.weight * 0.78).toFixed(3);
        }} else {{
      ...
      nodeElements.forEach(({{ work, group, label }}) => {{
        ...
        const strength = selectedConnections ? (selectedConnections.get(work.id) || 0) : 0;
        const nodeOpacity = strength > 0 ? (0.16 + strength * 0.84) : 0.08;
        const labelOpacity = strength > 0 ? (0.14 + strength * 0.82) : 0.08;
        group.classList.remove('dimmed');
        group.style.opacity = nodeOpacity.toFixed(3);
    ...
    zoomInBtn.addEventListener('click', () => applyZoom(zoomState.level + ZOOM_STEP));
    zoomOutBtn.addEventListener('click', () => applyZoom(zoomState.level - ZOOM_STEP));
    zoomResetBtn.addEventListener('click', () => applyZoom(1));
    window.addEventListener('resize', () => applyZoom(zoomState.level));
